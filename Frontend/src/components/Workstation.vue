...
async function fetchPendingOrders() {
  try {
    const res = await axios.get(`${import.meta.env.VITE_URL}/api/orders/pending`);
...
async function closeOrder(orderId, category) {
  ...
  try {
    ...
    const res = await axios.put(`${import.meta.env.VITE_URL}/api/orders/${orderId}/category/${category}/close`);
